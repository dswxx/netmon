...
* **挂载点**：
  ...
  * `udp_recvmsg`：使用 **Map-in-Map 上下文追踪技术**，在入口记录 Socket 信息，在出口统计接收字节数，解决 UDP 无连接接收统计难题。
* **CO-RE**：引入 `vmlinux.h`，摆脱对本地内核头文件的依赖，实现跨内核版本兼容。

### 2. 用户态（Go 代码）

* **数据清洗**：每秒读取 BPF Map，对同一 PID 下的所有线程数据进行聚合。
* **名字解析**：采用双重策略——优先读取 `/proc/PID/comm`，失败则使用 BPF 存储的历史名字，保证活跃与死亡进程均显示正确名称。
* **UI 渲染**：使用 `termui` 库构建响应式 TUI，实现嵌套网格布局和自适应图表组件。
